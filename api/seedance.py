"""
Seedance API Webhook Handler - Vercel Serverless Function
...
def post_to_meta(image_url, video_url, caption):
    ...
    try:
        ...
        if image_url:
            endpoint = f"https://graph.instagram.com/v21.0/me/media"
        ...
        if response.status_code in [200, 201]:
            logger.info(f"✅ Meta post successful")
            return True
        else:
...
def update_shopify_product(product_id, new_image_url):
    ...
    try:
        ...
        if response.status_code == 200:
            logger.info(f"✅ Shopify update successful")
            return True
        else:
...
def handler(event, context):
    ...
    if event['httpMethod'] != 'POST':
        if event['httpMethod'] == 'GET':
            return {'statusCode': 200, 'body': json.dumps({'status': 'operational'})}
    ...
    try:
        ...
        signature = event['headers'].get('x-seedance-signature', '')
        ...
        return {
            'statusCode': 200,
            'body': json.dumps({'status': 'success' if success else 'partial', 'event_id': payload.get('generation_id')})
        }
